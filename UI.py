...
class UI:
    ...
    def home() -> None:
        ...
        if "id_entregador_logado" not in st.session_state:
            st.session_state.id_entregador_logado = None
        if "tela" not in st.session_state:
            st.session_state.tela = "login"

        # Roteamento baseado no tipo de usuário logado
    ...
    # CRIANDO USUÁRIO (CLIENTE OU ENTREGADOR)
    @staticmethod
    def criar_usuario() -> None:                           
        ...
        if submit:
            ...
            st.success(f"Conta de {tipo_perfil} criada com sucesso! Faça login agora.")
            time.sleep(2)
            st.session_state.tela = "login" 
            st.rerun()
    ...
    def validacao() -> None:
        ...
        if button:
            if tipo_login == "Cliente / Admin":
                ...
                # Validação para Entregadores
                if LoginDAO.logado_entregador(email, senha):
                    st.success("Entregador logado com sucesso!")
                    st.session_state.usuario_logado = True
                    st.session_state.email_logado = email
                    ...
                    st.error("Credenciais de entregador incorretas!")


# Execução do ponto de entrada do App
UI.home()